...
@push('scripts')
    ...
    <script>
        ...
        var companyTimeFormat = typeof company !== 'undefined' && company && company.time_format ? company.time_format : '{{ company()->time_format }}';

        var calendar = new FullCalendar.Calendar(calendarEl, {
            locale: initialLocaleCode,
            timeZone: '{{ company()->timezone }}',
            firstDay: parseInt("{{ attendance_setting()?->week_start_from }}") || 0,
            headerToolbar: {
            ...
            selectable: false,
            selectMirror: true,
            noEventsContent: 'No events available',
            ...
            eventDidMount: function(info) {
                if (info.event.extendedProps && info.event.extendedProps.bg_color) {
                ...
                if (info.event.extendedProps && info.event.extendedProps.icon) {
                    $(info.el).find('td.fc-list-event-title').prepend('<i class="fa ' + info.event.extendedProps.icon + '"></i>&nbsp;&nbsp;');
                }
                // tooltip for leaves
                if (info.event.extendedProps && info.event.extendedProps.event_type == 'leave') {
            ...
            eventTimeFormat: {
                hour: companyTimeFormat == 'H:i' ? '2-digit' : 'numeric',
                minute: '2-digit',
                hour12: companyTimeFormat == 'H:i' ? false : true,
                meridiem: companyTimeFormat == 'H:i' ? false : true
            }
        });
