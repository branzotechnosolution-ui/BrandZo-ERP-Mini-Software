        ...
        <x-form id="save-employee-data-form">
            ...
            <div class="bg-white rounded add-client">
                ...
                <div class="p-20 row">
                        ...
                        <x-forms.select fieldId="country" :fieldLabel="__('app.country')" fieldName="country"
                            ...
                            @foreach ($countries as $item)
                                <option data-tokens="{{ $item->iso3 }}" data-phonecode = "{{$item->phonecode}}" data-iso="{{ $item->iso }}"
                                    ...
                                    value="{{ $item->id }}" {{ strtolower($item->iso) == 'in' ? 'selected' : '' }}>{{ $item->nicename }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    <div class="col-lg-3 col-md-6">
                            ...
                            <x-forms.select fieldId="country_phonecode" fieldName="country_phonecode"
                                ...
                                @foreach ($countries as $item)
                                    <option data-tokens="{{ $item->name }}" data-country-iso="{{ $item->iso }}"
                                            ...
                                            value="{{ $item->phonecode }}" {{ strtolower($item->iso) == 'in' ? 'selected' : '' }}>{{ $item->phonecode }}
                                    </option>
                                @endforeach
                            </x-forms.select>
                    ...
                    <div class="col-lg-3 col-md-6 district-field-wrapper">
...
<script>
    $(document).ready(function() {
        ...
        function isIndiaSelected() {
            return $('#country').find(':selected').data('iso') === 'IN' || $('#country').find(':selected').data('iso') === 'in';
        }

        function toggleStateDistrictFields() {
            if (isIndiaSelected()) {
                $('.state-field-wrapper, .district-field-wrapper').removeClass('d-none');
                loadIndiaStates();
            } else {
                // Non-India: allow free typing instead of the India-specific dropdown
                $('.state-field-wrapper, .district-field-wrapper').removeClass('d-none');
        ...
        $('#state').on('change', function () {
            if (isIndiaSelected()) {
                loadDistrictsForState($(this).val());
            }
        });

        $('#country').on('change', function () {
            toggleStateDistrictFields();
        });

        // Initialize on page load (India is the default selected country)
        toggleStateDistrictFields();
