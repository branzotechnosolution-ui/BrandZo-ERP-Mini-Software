...
@push('scripts')
    ...
    <script>
       ...
       function changeStage(leadID, elem) {
            var selectedStage = $(elem).find(':selected').text().trim();
            var selectedStageId = $(elem).val();
            var modalUrl = "{{ route('deals.follow_up', ':id') }}?stage=" + encodeURIComponent(selectedStage) + "&stage_id=" + encodeURIComponent(selectedStageId);
            modalUrl = modalUrl.replace(':id', leadID);
            $(MODAL_LG + ' ' + MODAL_HEADING).html('Update Follow-up');
        ...
        function convertDealToClient(dealID) {
            var url = "{{ route('deals.convert_to_client_modal', ':id') }}";
            url = url.replace(':id', dealID);
            .html('...');
            $.ajaxModal(MODAL_LG, url);
        }
.on('click', '#add-lead', function() {
