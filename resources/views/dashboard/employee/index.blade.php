            ...
            <div class="ml-auto d-flex flex-column align-items-end clock-in-out mb-3 mb-lg-0 mb-md-0 mt-4 mt-lg-0 mt-md-0">
                <p class="mb-2 text-right f-18 font-weight-bold text-dark-grey d-grid align-items-center">
                    ...
                    <span id="dashboard-clock">
                        {!! now()->timezone(company()->timezone)->translatedFormat(company()->time_format) . '</span><span class="f-10 font-weight-normal ml-1">' . now()->timezone(company()->timezone)->translatedFormat('l') . '</span>' !!}

                    @if (!is_null($currentClockIn))
                        <span class="f-11 font-weight-normal text-lightest d-block mt-1">
                            @lang('app.clockInAt') - {{ $currentClockIn->clock_in_time->timezone(company()->timezone)->translatedFormat(company()->time_format) }}
                        </span>
                    @endif
                </p>
