...
class ReportsController extends AccountBaseController
{
    ...
    public function show($id)
    {
        switch ($id) {
            case 'lead-report':
                return redirect()->route('lead-report.index');
            case 'sales-report':
                return redirect()->route('sales-report.index');
            case 'attendance-report':
                return redirect()->route('attendance-report.index');
            case 'timelog-report':
                return redirect()->route('time-log-report.index');
            case 'task-report':
                return redirect()->route('task-report.index');
            case 'finance-report':
                return redirect()->route('finance-report.index');
            case 'leave-report':
                return redirect()->route('leave-report.index');
            default:
                return $this->index();
        }
    }
