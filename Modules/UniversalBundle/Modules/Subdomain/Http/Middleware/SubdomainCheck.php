...
class SubdomainCheck
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        $rawHost = str_replace('www.', '', request()->getHttpHost());
        $host = explode(':', $rawHost)[0];
        $portSuffix = str_contains($rawHost, ':') ? ':' . explode(':', $rawHost)[1] : '';
        ...
        if ($routeName === 'login') {
            ...
            if ($root == $host) {
                return redirect('//' . $root . $portSuffix . '/signin');
            }
        ...
        if ($routeName == 'front.forgot-company') {
            return redirect('//' . $root . $portSuffix . '/forgot-company');
        }

        // Redirect to signup when from 325 page
        if ($routeName == 'front.signup.index') {
            return redirect('//' . $root . $portSuffix . '/signup');
        }
