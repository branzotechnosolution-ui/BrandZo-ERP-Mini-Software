...
<html lang="en">
    ...
    <body class="content-wrapper">
        ...
        @if (!is_null($jobOffer->annual_ctc))
            @php
                // Simple CTC breakdown: Basic = 50% of monthly CTC, HRA = 50% of Basic,
                // Take-home = remaining monthly CTC after Basic + HRA.
                $ctcMonthly = $jobOffer->annual_ctc / 12;
                $ctcBasic = $ctcMonthly * 0.5;
                $ctcHra = $ctcBasic * 0.5;
                $ctcTakeHome = $ctcMonthly - $ctcBasic - $ctcHra;
            @endphp
            <h5 class="f-14 text-black mt-2">@lang('recruit::modules.joboffer.ctcBreakdown')</h5>
            <table width="100%">
                <tbody>
                <tr>
                    <td>
                        <table class="f-14 b-collapse" width="100%">
                            ...
                            <tr class="main-table-heading text-grey">
                                <td width="40%">@lang('recruit::modules.joboffer.ctc') @lang('recruit::modules.joboffer.payAcc') @lang('recruit::app.job.month') ({{ $currency->currency_code ?? '' }})</td>
                                <td align="right">@lang('app.amount')</td>
                            </tr>
                            <tr style="border-bottom: 1px solid #DBDBDB;">
                                <td>@lang('recruit::modules.joboffer.basicPay')</td>
                                <td class="text-right" align="right">{{ currency_format($ctcBasic, $currency->id ?? null, false) }}</td>
                            </tr>
                            <tr style="border-bottom: 1px solid #DBDBDB;">
                                <td>@lang('recruit::modules.joboffer.hra')</td>
                                <td class="text-right" align="right">{{ currency_format($ctcHra, $currency->id ?? null, false) }}</td>
                            </tr>
                            <tr class="text-black text-bold line-height">
                                <td>@lang('recruit::modules.joboffer.takeHomeMonthly')</td>
                                <td align="right">{{ currency_format($ctcTakeHome, $currency->id ?? null, false) }}</td>
                            </tr>
                            <tr class="text-black text-bold line-height">
                                <td>@lang('recruit::modules.joboffer.annualCtc')</td>
                                <td align="right">{{ currency_format($jobOffer->annual_ctc, $currency->id ?? null, false) }}</td>
                            </tr>
                        </table>
                    </td>
                </tr>
        ...
        <table class="bg-white mt-2" border="0" cellpadding="0" cellspacing="0" width="100%" role="presentation">
            <tr>
                <td style="border-top: 1px solid #DBDBDB;" colspan="2"></td>
            </tr>
            <tr>
                <td class="f-14 text-grey" width="60%" style="vertical-align: bottom;">
                    @if (!empty($company->address))
                        <p class="line-height mb-0">{{ $company->address }}</p>
                    @endif
                </td>
                <td width="40%" align="right">
                    @if (!is_null($company->signature_url))
                        <div style="text-align: right;">
                            {!! Html::image($company->signature_url, '', ['height' => '60px']) !!}
                            <p class="f-14 text-black mb-0">For {{ $company->company_name }}</p>
                            <p class="f-14 text-grey mb-0">@lang('app.signature')</p>
                        </div>
                    @endif
                </td>
            </tr>
