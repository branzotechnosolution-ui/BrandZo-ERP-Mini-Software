...
<script>
    function handleHrmsLogoutGuard(event) {
        if (event) event.preventDefault();
        $.ajax({
            url: "<?php echo e(route('attendances.today_summary')); ?>",
            type: "GET",
            dataType: "json",
                ...
                if (response && response.status === 'success') {
                    if (response.is_on_break) {
                        Swal.fire({
                            ...
                            text: 'Please end your break before logout.',
                    ...
                    if (response.is_clocked_in && !response.is_clocked_out) {
                        if (typeof hrmsOpenCheckoutModal === 'function') {
                            hrmsOpenCheckoutModal();
                        } else if ($('#hrmsCheckoutModal').length) {
                            $('#hrmsCheckoutModal').modal('show');
                        } else {
                            let modalWork = response.total_work_formatted || '00h 00m';
                            let modalBreak = response.total_break_formatted || '00h 00m';
                            let modalNet = response.net_work_formatted || '00h 00m';
                            let clockInTime = response.clock_in_time || '--:--';
                            let loginTime = response.login_time || '--:--';

                            Swal.fire({
                                title: '<i class="fa fa-user-clock text-primary mr-2"></i>Attendance Summary',
                                html: '<div class="text-left font-weight-semibold f-14 p-3 bg-light rounded border border-grey">' +
                                      '<p class="mb-2"><strong>Login Time:</strong> ' + loginTime + '</p>' +
                                      '<p class="mb-2"><strong>Check In Time:</strong> ' + clockInTime + '</p>' +
                                      '<p class="mb-2"><strong>Logout Time:</strong> ' + response.clock_out_time + '</p>' +
                                      '<p class="mb-2 text-success"><strong>Total Work Duration:</strong> ' + modalWork + '</p>' +
                                      '<p class="mb-2 text-warning"><strong>Total Break Duration:</strong> ' + modalBreak + '</p>' +
                                      '<p class="mb-0 text-info"><strong>Net Working Hours:</strong> ' + modalNet + '</p>' +
                                      '</div>',
                                showCancelButton: true,
                                confirmButtonText: 'Confirm Check Out & Logout',
                                ...
                                if (result.isConfirmed) {
                                    $.ajax({
                                        url: "<?php echo e(route('attendances.quick_clock_out')); ?>",
                                        type: "POST",
...
</script><?php /**PATH /Users/macbook/Downloads/BrandZo_ERP_Mini_Software 3/resources/views/sections/topbar.blade.php ENDPATH**/ ?>