...
<?php endif; ?>
        ...
        <div class="d-lg-flex d-md-flex d-block py-2 pb-2 align-items-center">
            ...
            <div class="ml-auto d-flex flex-column align-items-end clock-in-out mb-3 mb-lg-0 mb-md-0 mt-4 mt-lg-0 mt-md-0">
                <p class="mb-2 text-right f-18 font-weight-bold text-dark-grey d-grid align-items-center">
                    ...
                    <span id="dashboard-clock">
                        <?php echo now()->timezone(company()->timezone)->translatedFormat(company()->time_format) . '</span><span class="f-10 font-weight-normal ml-1">' . now()->timezone(company()->timezone)->translatedFormat('l') . '</span>'; ?>


                    <?php if(!is_null($currentClockIn)): ?>
                        <span class="f-11 font-weight-normal text-lightest d-block mt-1">
                            <?php echo app('translator')->get('app.clockInAt'); ?> - <?php echo e($currentClockIn->clock_in_time->timezone(company()->timezone)->translatedFormat(company()->time_format)); ?>
                ...
                <?php
                    $attUser = user();
                    $attTz = company()->timezone;
                    $attToday = now($attTz)->toDateString();

                    $allAttendances = \App\Models\Attendance::with(['breaks', 'activeBreak'])
                        ->where('user_id', $attUser->id)
                        ->whereDate('clock_in_time', $attToday)
                        ->orderBy('clock_in_time', 'asc')
                        ->get();

                    $attendanceRecord = \App\Models\Attendance::with(['breaks', 'activeBreak'])
                        ->where('user_id', $attUser->id)
                        ->whereDate('clock_in_time', $attToday)
                    ...
                    $isClockedIn = $attendanceRecord && is_null($attendanceRecord->clock_out_time);
                    ...
                    $isOnBreak = $attendanceRecord && !is_null($activeBreak);
                    ...
                    if ($start_time->gt($end_time)) {
                        if (now(company()->timezone)->lessThan($end_time) || (now(company()->timezone)->greaterThan($end_time) && now(company()->timezone)->lessThan($start_time))) {
                            $start_time->subDay();
                            $mid_time->subDay();
                        } else {
                            $mid_time->addDay();
                    ...
                    $isDefaultShiftAssignedToday = $defaultShift ? in_array($currentDayOfWeek, json_decode($defaultShift->office_open_days, true) ?? []) : true;
                ...
                <!-- HRMS HEADER ACTION BUTTONS NEAR CLOCK -->
                <div class="d-flex align-items-center gap-2 mt-1" id="hrms-header-action-buttons">
                    <?php if(!$isClockedIn): ?>
                        <!-- NOT CHECKED IN STATE (Initial / Post-Checkout): SHOW ONLY Green Login button -->
                        <button type="button" class="btn btn-success text-white rounded-pill font-weight-bold px-4 py-2 btn-hrms-action" id="btn-hrms-checkin" onclick="hrmsAction('checkin')" style="background-color: #10B981; border: none;">
                            <i class="fa fa-sign-in-alt mr-2"></i>Login
                        </button>
                    <?php elseif($isClockedIn && !$isOnBreak): ?>
                        <!-- WORKING STATE: SHOW Orange Start Break & Red Logout -->
                        <button type="button" class="btn btn-warning text-white rounded-pill font-weight-bold px-4 py-2 mr-2 btn-hrms-action" id="btn-hrms-startbreak" onclick="hrmsAction('startbreak')" style="background-color: #F59E0B; border: none;">
                            <i class="fa fa-coffee mr-2"></i>Start Break
                        </button>
                        <button type="button" class="btn btn-danger rounded-pill font-weight-bold px-4 py-2 btn-hrms-action" id="btn-hrms-checkout" onclick="hrmsOpenCheckoutModal()" style="background-color: #EF4444; border: none;">
                            <i class="fa fa-sign-out-alt mr-2"></i>Logout
                        </button>
                    <?php elseif($isClockedIn && $isOnBreak): ?>
                        <!-- ON BREAK STATE: SHOW ONLY Blue End Break -->
                        <button type="button" class="btn btn-primary text-white rounded-pill font-weight-bold px-4 py-2 btn-hrms-action" id="btn-hrms-endbreak" onclick="hrmsAction('endbreak')" style="background-color: #2563EB; border: none;">
            ...
            <?php if(in_array('admin', user_roles())): ?>
                <div class="private-dash-settings d-flex align-self-center ml-3">
...
<?php endif; ?>
        ...
        <!-- HRMS ATTENDANCE CARD MODULE START -->
        <div class="row mb-3">
            <div class="col-md-12">
                <?php if (isset($component)) { $__componentOriginal737f270eeb9a1e8da112dc7c5cacc168 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal737f270eeb9a1e8da112dc7c5cacc168 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.hrms-attendance-card','data' => []] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
<?php $component->withName('hrms-attendance-card'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag && $constructor = (new ReflectionClass(Illuminate\View\AnonymousComponent::class))->getConstructor()): ?>
<?php $attributes = $attributes->except(collect($constructor->getParameters())->map->getName()->all()); ?>
...
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginal737f270eeb9a1e8da112dc7c5cacc168)): ?>
<?php $attributes = $__attributesOriginal737f270eeb9a1e8da112dc7c5cacc168; ?>
<?php unset($__attributesOriginal737f270eeb9a1e8da112dc7c5cacc168); ?>
<?php endif; ?>
<?php if (isset($__componentOriginal737f270eeb9a1e8da112dc7c5cacc168)): ?>
<?php $component = $__componentOriginal737f270eeb9a1e8da112dc7c5cacc168; ?>
<?php unset($__componentOriginal737f270eeb9a1e8da112dc7c5cacc168); ?>
<?php endif; ?>
            </div>
        </div>
        <!-- HRMS ATTENDANCE CARD MODULE END -->
...
<?php endif; ?>
            ...
            <div class="col-xl-7 col-lg-12 col-md-12 e-d-tasks-projects-events">
                ...
                <div class="row mb-3 mt-xl-0 mt-lg-4 mt-md-4 mt-4">
                    ...
                    <?php echo $__env->make('dashboard.employee.widgets.documents', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                </div>
    ...
    <?php if((!is_null($viewEventPermission) && $viewEventPermission != 'none')
        ...
        <script>
            ...
            if (clockInButton) {
                ...
                var shiftAssigned = "<?php echo e($shiftAssigned?->exists); ?>";
                var allowOutsideShift = "<?php echo e(attendance_setting()->show_clock_in_button); ?>";
                var isDefaultShiftAssignedToday = "<?php echo e($isDefaultShiftAssignedToday); ?>";
            ...
            var getEventDetail = function(id,type) {
                ...
                switch (type) {
                    ...
                    case 'leave':
                        ...
                        break;
                    case 'follow_up':
                        var url = "<?php echo e(route('deals.show', ':id')); ?>";
                        break;
                    default:
                        return 0;
                        break;
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/macbook/Downloads/BrandZo_ERP_Mini_Software 3/resources/views/dashboard/employee/index.blade.php ENDPATH**/ ?>