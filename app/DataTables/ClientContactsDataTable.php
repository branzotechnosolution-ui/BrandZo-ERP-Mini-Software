...
class ClientContactsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('title', fn($row) => $row->title ?: 'Primary Contact')
            ->addColumn('whatsapp', function ($row) {
                $num = preg_replace('/[^0-9]/', '', $row->phone ?: '');
                return $row->phone ? '<a href="https://wa.me/' . $num . '" target="_blank" class="text-success font-weight-bold"><i class="fab fa-whatsapp mr-1"></i>' . $row->phone . '</a>' : '--';
            })
            ->editColumn('created_at', fn($row) => Carbon::parse($row->created_at)->translatedFormat($this->company->date_format))
            ->addIndexColumn()
            ->smart(false)
            ->setRowId(fn($row) => 'row-' . $row->id)
            ->rawColumns(['contact_name', 'action', 'check', 'whatsapp']);
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.title') => ['data' => 'title', 'name' => 'title', 'title' => 'Designation / Title'],
            __('app.email') => ['data' => 'email', 'name' => 'email', 'title' => __('app.email')],
            __('app.phone') => ['data' => 'phone', 'name' => 'phone', 'title' => __('app.phone')],
            'WhatsApp' => ['data' => 'whatsapp', 'name' => 'phone', 'title' => 'WhatsApp'],
