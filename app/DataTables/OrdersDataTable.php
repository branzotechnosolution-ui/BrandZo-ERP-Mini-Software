...
<a href="' . route('orders.show', [$row->id]) . '"
            ...
            if (
                !in_array('client', user_roles()) && !in_array($row->status, ['completed', 'refunded']) &&
        ...
        $datatables->editColumn('order_number', function ($row) {
            $clientCode = $row->client?->clientDetails?->client_code ?: ('BZCL' . str_pad($row->client_id ?: 0, 3, '0', STR_PAD_LEFT));
            $cNum = preg_replace('/[^0-9]/', '', $clientCode) ?: str_pad($row->client_id ?: 0, 3, '0', STR_PAD_LEFT);
            $ordCode = 'BZORD' . str_pad($cNum, 3, '0', STR_PAD_LEFT) . '-' . str_pad($row->id, 3, '0', STR_PAD_LEFT);

            return '<div class="media align-items-center">
                        <div class="media-body">
                    <h5 class="mb-0 f-13 text-darkest-grey"><a href="' . route('orders.show', [$row->id]) . '" class="font-weight-bold">' . $ordCode . '</a></h5>
                    </div>
                  </div>';
        });
