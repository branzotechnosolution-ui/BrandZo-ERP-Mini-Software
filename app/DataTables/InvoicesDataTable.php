...
class InvoicesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('invoice_number', function ($row) {
            ...
            $invCode = 'BZINV' . str_pad($cNum, 3, '0', STR_PAD_LEFT) . '-' . str_pad($row->id, 3, '0', STR_PAD_LEFT);

            return '<div class="media align-items-center">
                        <div class="media-body">
                    <h5 class="mb-0 f-13 text-darkest-grey"><a href="' . route('invoices.show', [$row->id]) . '" class="font-weight-bold">' . $invCode . '</a></h5><p class="mb-0">' . $recurring . '</p>
                    </div>
                  </div>';
        });
