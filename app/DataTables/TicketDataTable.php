...
class   TicketDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('subject', function($row) {
            $clientCode = $row->requester?->clientDetails?->client_code ?: ('BZCL' . str_pad($row->user_id ?: 0, 3, '0', STR_PAD_LEFT));
            $cNum = preg_replace('/[^0-9]/', '', $clientCode) ?: str_pad($row->user_id ?: 0, 3, '0', STR_PAD_LEFT);
            $tktCode = 'BZTKT' . str_pad($cNum, 3, '0', STR_PAD_LEFT) . '-' . str_pad($row->id, 3, '0', STR_PAD_LEFT);

            return '<div class="media align-items-center"><div class="media-body"><span class="badge badge-secondary mr-2 font-weight-bold">' . $tktCode . '</span><a href="' . route('tickets.show', $row->ticket_number) . '" class="text-darkest-grey font-weight-bold">' . $row->subject . '</a>' . $row->badge() . '</div></div>';
        });
