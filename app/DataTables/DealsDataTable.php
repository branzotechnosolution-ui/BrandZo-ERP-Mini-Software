...
class DealsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            if (!in_array($row->leadStage?->slug, ['win', 'converted']) && $row->create_client != 'yes') {
                $action .= '<a class="dropdown-item openRightModal" href="' . route('deals.convert_to_client_modal', [$row->id]) . '">
        ...
        $datatables->addColumn('export_client_name', function ($row) {
            ...
            if (!empty($row->salutation)) {
                try {
                    $salutationLabel = ($row->salutation instanceof Salutation) ? $row->salutation->label() : (Salutation::tryFrom($row->salutation)?->label() ?: $row->salutation);
                    $client_name = trim($salutationLabel . ' ' . $client_name);
                } catch (\Throwable $e) {}
            }
            return $client_name;
        });
        ...
        $datatables->addColumn('client_name', function ($row) {
            ...
            $contactUrl = $row->contact_id ? route('lead-contact.show', [$row->contact_id]) : 'javascript:;';

            return '
                        <div class="media-body">
                    <h5 class="mb-0 f-13 text-truncate"><a href="' . $contactUrl . '">' . str($client_name)->limit(18) . '</a></h5>
        ...
        $datatables->editColumn('name', function ($row) {
            $dealCode = $row->deal_code ?: ('BZ' . str_pad($row->id, 3, '0', STR_PAD_LEFT));
            $dealIdBadge = '<span class="badge badge-primary px-2 py-1 mr-1 font-weight-bold">' . $dealCode . '</span>';
            return '
                        <div class="media-body">
                            <h5 class="mb-0 f-13 ">' . $dealIdBadge . '<a href="' . route('deals.show', [$row->id]) . '" class="text-darkest-grey font-weight-bold">' . $row->name . '</a></h5>
                        </div>
                  ';
        });
    ...
    public function query(Deal $model)
    {
        $lead = $model->with([
            ...
            ->select(
                ...
                'users.image',
                DB::raw("COALESCE(NULLIF(deals.company_name, ''), leads.company_name) as company_name"),
                DB::raw("COALESCE(NULLIF(deals.mobile, ''), leads.mobile) as mobile"),
                DB::raw("COALESCE(leads.salutation, '') as salutation"),
                'deals.lead_contact_id as contact_id',
                DB::raw("COALESCE(NULLIF(deals.client_name, ''), leads.client_name) as client_name"),
                DB::raw("COALESCE(NULLIF(deals.client_email, ''), leads.client_email) as client_email"),
            ...
            ->leftJoin('leads', 'leads.id', 'deals.lead_contact_id')
        ...
        if ($this->request()->type != 'all' && $this->request()->type != '') {
            ...
            if ($this->request()->type == 'lead') {
                ...
                $lead = $lead->where('deals.create_client', 'yes');
            } else {
                $lead = $lead->whereNotNull('leads.client_id');
            }
        } else {
            $lead = $lead->where(function($q) {
                $q->whereNull('deals.create_client')
                  ->orWhere('deals.create_client', '!=', 'yes');
            });
        }
        ...
        if ($this->request()->leadId !== null && $this->request()->leadId != 'null' && $this->request()->leadId != '' && $this->request()->leadId != 'all') {
            $lead = $lead->where('deals.lead_contact_id', $this->request()->leadId);
        }
        ...
        if ($this->request()->lead_agent_id !== null && $this->request()->lead_agent_id != 'null' && $this->request()->lead_agent_id != '' && $this->request()->lead_agent_id != 'all') {
            $lead = $lead->where('deals.agent_id', $this->request()->lead_agent_id);
        }
        ...
        if ($this->request()->searchText != '') {
            $lead = $lead->where(function ($query) {
                ...
                $query->where('leads.client_name', 'like', '%' . $safeTerm . '%')
                    ...
                    ->orWhere('deals.deal_code', 'like', '%' . $safeTerm . '%')
                    ->orWhere('deals.id', 'like', '%' . $safeTerm . '%');
            });
        }
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('modules.deal.dealValue') => ['data' => 'lead_value', 'name' => 'value', 'title' => __('app.value'), 'exportable' => false, 'visible' => false],
