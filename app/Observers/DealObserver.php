...
class DealObserver
{
    ...
    public function updated(Deal $deal)
    {
        if (!isRunningInConsoleOrSeeding()) {
            ...
            if ($deal->isDirty('agent_id')) {
                try { event(new DealEvent($deal, $deal->leadAgent, 'LeadAgentAssigned')); } catch (\Throwable $e) {}
            }

            if ($deal->isDirty('pipeline_stage_id') || $deal->isDirty('lead_pipeline_id')) {
                try { event(new DealEvent($deal, $deal->leadAgent, 'StageUpdated')); } catch (\Throwable $e) {}
            }
        }
    }
                ...
                if (!session()->has('is_imported') && !session()->has('create_deal_with_lead')) {
                    ...
                    if (request('agent_id') != '') {
                        try {
                            ...
                            \Illuminate\Support\Facades\Log::error('Failed to dispatch LeadAgentAssigned event: ' . $e->getMessage());
                        }
                    } else {
                        try {
                            Notification::send(User::allAdmins($deal->company->id), new LeadAgentAssigned($deal));
                        } catch (\Throwable $e) {
                            \Illuminate\Support\Facades\Log::error('Failed to send LeadAgentAssigned notification: ' . $e->getMessage());
                        }
                    }
    ...
    private function createClient($deal)
    {
        ...
        if ($deal->create_client == 1 && $deal->pipeline_stage_id == $stage?->id) {
            ...
            try {
                $existingUser = User::where('email', $lead->client_email)->first();
                if ($existingUser) {
                ...
                \Illuminate\Support\Facades\Log::error('createClient in DealObserver error: ' . $e->getMessage());
            }
        }
    }
}
