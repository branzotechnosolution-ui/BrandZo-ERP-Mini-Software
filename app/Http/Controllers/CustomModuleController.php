...
class CustomModuleController extends AccountBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        if (!file_exists(storage_path('app') . '/Modules/' . $moduleName . '/Config/config.php')) {
            $dirs = File::directories(storage_path('app') . '/Modules');
            foreach ($dirs as $dir) {
                if (file_exists($dir . '/Config/config.php') || file_exists($dir . '/module.json')) {
    ...
    public function validateModule($moduleName)
    {
        ...
        $currentAppVersion = trim(File::exists(public_path('version.txt')) ? File::get(public_path('version.txt')) : (File::exists(base_path('version.txt')) ? File::get(base_path('version.txt')) : '5.5.20'));

        if (version_compare($config['parent_min_version'], $currentAppVersion, '>')) {
            return [
                'status' => false,
                'message' => 'Minimum version of <b>' . $appName . ' main application</b> should be greater than or equal to <b>' . $config['parent_min_version'] . '</b>. Your application version is <b>' . $currentAppVersion . '</b>'
            ];
        }
    ...
    private function updateVersion($moduleName)
    {
        try {
            ...
            if (isset($config['setting']) && !is_null($config['setting']) && class_exists($config['setting'])) {
                $setting = ($config['setting'])::first();
