...
class AttendanceController extends AccountBaseController
{
    ...
    public function quickClockIn(Request $request)
    {
        try {
            ...
            return response()->json(['status' => 'success', 'message' => __('Working (Checked In) successfully!')]);
        } catch (\Throwable $e) {
            return response()->json(['status' => 'error', 'message' => 'Clock In failed: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function quickClockOut(Request $request)
    {
        try {
            $user = user();
            $today = now()->toDateString();
            $attendance = Attendance::with(['activeBreak', 'breaks'])
                ->where('user_id', $user->id)
                ->whereDate('clock_in_time', $today)
                ->whereNull('clock_out_time')
                ->first();
            ...
            return response()->json(['status' => 'success', 'message' => __('Checked Out successfully!')]);
        } catch (\Throwable $e) {
            return response()->json(['status' => 'error', 'message' => 'Check Out failed: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function startBreak(Request $request)
    {
        try {
            ...
            return response()->json(['status' => 'success', 'message' => __('Break Started Successfully')]);
        } catch (\Throwable $e) {
            return response()->json(['status' => 'error', 'message' => 'Break start failed: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function endBreak(Request $request)
    {
        try {
            ...
            if (!$activeBreak) {
                return response()->json(['status' => 'error', 'message' => __('No active break found to end.')], 400);
            }

            $now = now();
            $start = $activeBreak->start_time;
            $totalMinutes = $start ? $start->diffInMinutes($now) : 0;
            ...
            return response()->json(['status' => 'success', 'message' => __('Break Ended Successfully')]);
        } catch (\Throwable $e) {
            return response()->json(['status' => 'error', 'message' => 'Break end failed: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function todaySummary(Request $request)
    {
        ...
        $allAttendances = Attendance::with(['breaks', 'activeBreak'])
            ->where('user_id', $user->id)
            ->whereDate('clock_in_time', $today)
            ->orderBy('clock_in_time', 'asc')
            ->get();

        $activeAttendance = Attendance::with(['breaks', 'activeBreak'])
            ->where('user_id', $user->id)
            ->whereDate('clock_in_time', $today)
            ->whereNull('clock_out_time')
            ->latest('id')
        ...
        $isClockedIn = $activeAttendance && is_null($activeAttendance->clock_out_time);
        $isClockedOut = !$isClockedIn && $allAttendances->count() > 0 && !is_null($allAttendances->last()->clock_out_time);
        $activeBreak = $activeAttendance?->activeBreak;
        $isOnBreak = $activeAttendance && !is_null($activeBreak);
        ...
        return response()->json([
            ...
            'net_seconds' => max(0, $workSeconds - $totalBreakSeconds),
