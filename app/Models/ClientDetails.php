...
class ClientDetails extends BaseModel
{
    ...
    protected $fillable = ['client_id', 'company_name', 'user_id', 'address', 'postal_code', 'state', 'city', 'office', 'cell', 'website', 'note', 'skype', 'facebook', 'twitter', 'linkedin', 'tax_name', 'gst_number', 'shipping_address', 'category_id', 'sub_category_id', 'company_logo', 'electronic_address', 'electronic_address_scheme', 'whatsapp', 'pan_number', 'industry', 'company_size', 'lead_source', 'project_name', 'project_type', 'budget'];
    ...
    protected static function boot()
    {
        ...
        static::creating(function ($model) {
            if (empty($model->client_id)) {
                $userId = $model->user_id ?: ((\DB::table('users')->max('id') ?: 0) + 1);
                $model->client_id = 'BZCL' . str_pad($userId, 3, '0', STR_PAD_LEFT);
            }
        });
    }
    ...
    public function getClientCodeAttribute(): string
    {
        ...
        return 'BZCL' . str_pad($this->user_id ?: $this->id, 3, '0', STR_PAD_LEFT);
    }
